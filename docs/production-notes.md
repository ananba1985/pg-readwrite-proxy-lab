...
- 一键入口必须先收集完全部地址、端口、CIDR、现有业务账号与两类密码，然后才启动检查；检查阶段只允许 `/var/tmp` 下 root-only 临时配置/随机凭据、临时解压、SSH `known_hosts` 与远端暂存，不允许覆盖解压目录的既有运行配置，也不允许修改数据库、服务、账号或防火墙。只有 `APPLY` 后才备份并落盘新配置。
- Primary 必须校验厂商运行时与文件哈希、非恢复态、配置/HBA 可解析、数据库超级用户权限、业务账号/密码/探针、零业务连接、无自定义表空间、无外来复制连接/冲突槽、PGDATA 权限与可读性、备份容量及三机网络。
- Standby 必须校验同版厂商运行时、当前角色与 system identifier、零客户端连接、PGDATA/HOME/挂载和安全移动条件、旧数据保留容量、新基础备份容量以及到 Primary/Pgpool 的网络。
...
- 任一门禁失败必须退出并清理本次生成的临时凭据及远端暂存；不得提供“忽略并继续”开关。只有三台服务器都返回 `READY` 后，才打印变更摘要并接受人工输入 `APPLY`。
- 门禁是部署瞬间的证据，不替代变更审批、维护窗口、完整备份、恢复预案和回退负责人。输入 `APPLY` 后，脚本会在停止既有 Pgpool 前再次复核前端连接，在停止后、落盘配置前要求数据库普通客户端连接归零，并由 Primary/Standby 的变更脚本在本机实际改动前再检查一次；如果之后外部状态变化或执行失败，应按脚本给出的备份路径恢复，不得无条件重跑。
