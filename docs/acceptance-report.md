...
> 历史性说明：本报告记录的是 CentOS 数据库节点与 Pgpool TCP/9999 的旧实机验收。当前安装器的新基线为三台 Kylin Linux Advanced Server V10 aarch64、Pgpool TCP/5432，并支持启动参数、直接密码参数、Primary/Standby 共用的非标准 SSH 端口，以及数据库非零连接时的一次性人工强制重启菜单；不包含 9999 迁移或兼容监听。当前安装器也已完全移除主机防火墙检查与配置，本文后续 firewalld 记录仅为历史事实，不代表当前行为。这些新变更不冒充本报告中的历史实机证据，上线前需在目标环境重新验收。
