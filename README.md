...
参数同时支持 `--name value` 和 `--name=value`。脚本不回显密码；包含空格或 Shell 特殊字符的值应使用单引号。使用 `sudo bash install.sh --help` 可查看完整参数。可以只传一部分参数，未传入的项目才会交互补全；全部参数齐全时不会出现配置输入提示。旧命令中的 `--manage-pgpool-firewall yes|no` 仍可传入以保持兼容，但会被明确忽略，不触发任何防火墙检查或操作；新命令应直接删除该参数。
...
- 允许写入 Pgpool `pool_hba.conf` 的客户端 IPv4 CIDR；主机防火墙和上游 ACL 由用户自行配置；
- 已存在的业务用户名和数据库名；
